...
public class Array extends DataStructure {
     ...
     */
    public int[] getSize () {
        return StructParser.parseSize(this);
    }
    ...
    private void init (OP_ReadWrite rw) {
        ...
        if (size == null) { // Fall back to size declared in header
            size = getSize();
        }
        ...
        for (linearIndex++; linearIndex < linearTotal; linearIndex++) {
            IndexedElement ae = new IndexedElement(0.0, getIndexInNDimensions(linearIndex, size));
    ...
    public int[] getIndexInNDimensions (int linearIndex, final int[] dimensionSizes) {
         ...
         * <br>
         * Matrix has size, n by m. That is i = [0, n-1] and j = [0, m-1]. matrix[i][j] =
         * array[i*m + j]. For higher dimension, this idea generalizes, i.e. for a 3D
    ...
    /**
     * Calculate the product of all lower (to the right) dimension sizes. That is, for dim
     ...
     * <br>
     * Single-dimension Arrays default to {@link VisualType#bar} , higher dimension Arrays
     * default to {@link VisualType#box}. This method will always return a type.
     *
